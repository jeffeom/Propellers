...
extension MessengerViewController {
  ...
  func createTimeStamp(messageDate dateInInt: Int64) {
    guard let messageDate = Date(milliseconds: dateInInt) else { return }
    let currentDate = Date()
    let dateDiff = Calendar.current.dateComponents([.day], from: messageDate, to: currentDate)
    if dateDiff.day == 0 {
